...
object main extends App {
 ...
 weatherSample.foreach(println)
  ...
  val minTempArray: Array[Row] = weatherDataFrame.select(min("temperaturemin")).take(1)
  val maxTempArray: Array[Row] = weatherDataFrame.select(max("temperaturemax")).take(1)
  ...
  val weatherDataFrameWithYear = weatherDataFrame
  ...
  val aggregatedDF: DataFrame = weatherDataFrameWithYear
    .groupBy("year")
    .agg(min("temperaturemin"),max("temperaturemax"))
    .withColumnRenamed("min(temperaturemin)","temperaturemin")
    .withColumnRenamed("max(temperaturemax)","temperaturemax")
    .sort(asc("year"))
