...
// Suppress the log messages:
  ...
  // You are given an dataRDD of Rows (first element is x and the other y). We are aiming at finding simple linear 
  // regression model for the dataset using MLlib. I.e. find function f so that y ~ f(x)

  val hugeSequenceOfxyData = Seq(Row(0.0, 0.0), Row(0.3, 0.5), Row(0.9, 0.8), Row(1.0, 0.8), Row(2.0, 2.2),
      Row(2.2, 2.4), Row(3.0, 3.7), Row(4.0, 4.3))
  ...
  val allPredictions: DataFrame = lrModel
    .transform(vectorAssembler.transform(dataDF))
  allPredictions.show
  

  // Task #5: Use the LinearRegressionModel to predict y for values [-0.5, 3.14, 7.0]
  val rows = Seq(Row(-0.5, 0.0), Row(3.14, 0.0), Row(7.0, 0.0))
  val newRDD = spark.sparkContext.parallelize(rows)
  val newDF: DataFrame = spark.createDataFrame(newRDD, schema)
  
  val newPredictions: DataFrame = lrModel
    .transform(vectorAssembler.transform(newDF))
  newPredictions.show
